...
function maskLicenseCode(code: string, revealed: boolean): string {
  ...
  // First segment stays visible; every non-dash character in the rest becomes *
  return code
    .split("-")
...
function InnerLicensesPage() {
  ...
  // ── Resolve the Cognito sub so cache keys are user-scoped ─────────────────
  useEffect(() => {
    let cancelled = false;
    fetchUserAttributes()
      .then((attrs) => {
  ...
  // ── Auto-fetch on mount once the sub is known and no cached data exists ───
  useEffect(() => {
    if (!userSub) return;
    if (!licenseCache.isLoaded) return;
    // If there's already cached data and we're on cooldown, don't re-fetch
    if (licenseCache.value && licenseCache.isOnCooldown) return;

    getLicense();
    // We only want this to run once after sub + isLoaded are both ready
  ...
  async function regenerateLicense() {
    ...
    try {
      ...
      // Update both caches — regenerate returns fresh license data
  ...
  const maskedCode = license
    ? maskLicenseCode(license.license_code, revealed)
    : null;
    ...
    <main className="mx-auto max-w-4xl px-4 py-10 space-y-8">
      ...
      {maskedCode && (
        <div className="rounded-xl border border-border bg-card p-5 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* Clickable span — copies the real (unmasked) code */}
            <span
      ...
      {!isLoading && license && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900/60 dark:bg-amber-950/30 px-4 py-3 text-sm text-amber-800 dark:text-amber-300 space-y-1">
          <p>
            <strong>Note:</strong> Offline-activated machines cannot be removed
            from your license — neither by regenerating your license code nor by
            deactivating via the app.
          </p>
      ...
      {/* ── Action buttons ── */}
      <div className="flex flex-wrap items-center gap-3">
        {/* Manual refresh — only shown when not on cooldown */}
        {!licenseCache.isOnCooldown && (
          <Button
            variant="outline"
            onClick={getLicense}
          ...
          >
            <RefreshCw
              className={`size-4 mr-2 ${busyAction === "get" ? "animate-spin" : ""}`}
            />
        ...
        >
          <RotateCcw
            className={`size-4 mr-2 ${busyAction === "regenerate" ? "animate-spin" : ""}`}
          />
          {busyAction === "regenerate" ? "Regenerating…" : "Regenerate License"}
        </Button>

        {/* Regenerate cooldown countdown */}
...
// ─── Exported page (gated behind auth) ───────────────────────────────────────
